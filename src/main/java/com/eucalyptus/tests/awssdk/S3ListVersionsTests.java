...
/**
 * <p>
 * This class contains tests for listing object versions in a bucket.
 * </p>
 * 
 * <p>
 * All tests are passing after the commit 2841c3707b6b918bd84b2e849d107d154ad2a2e2
 * </p>
 * 
...
public class S3ListVersionsTests {
  ...
  /**
   * <p>
   * Test for verifying ordering of version information for a single key in a bucket
   * </p>
   * 
   * <p>
   * This test uploads an object using the same key multiple times. It fetches the version information for the bucket and verifies it for
  ...
  /**
   * <p>
   * Test for verifying ordering of version information for multiple keys in a bucket
   * </p>
   * 
   * <p>
   * This test uploads objects using the different keys multiple times. It fetches the version information for the bucket and verifies it for
   * lexicographic ordering of keys and chronological ordering of versions
   * </p>
   */
  @Test
  ...
  /**
   * <p>
   * Test for verifying delete markers and verifying the ordering of version information for a single key in a bucket
   * </p>
   * 
   * <p>
   * This test uploads an object using the same key multiple times, deletes the object without specifying the version and repeats the upload step
   * again. After each action, the test fetches the version information for the bucket and verifies it for chronological ordering of versions and
   * delete markers.
   * </p>
   */
  @Test
      ...
      cleanupTasks.add(new Runnable() {
        ...
        public void run() {
          for (String versionId : deleteMarkers) {
            print(account + ": Deleting delete marker for key " + key + ", with version " + versionId);
  ...
  /**
   * <p>
   * Test for verifying that a unique delete marker is returned every time an object is deleted, irrespective of the number of times the object is
   * deleted, in a bucket with versioning state set to enabled or suspended
   * </p>
   */
  @Test
  public void uniqueDeleteMarker() throws Exception {
    ...
    try {
      ...
      // Suspend versioning
      print(account + ": Suspending versioning");
  ...
  /**
   * <p>
   * Test for listing and verifying object versions using key-marker and no version-id-marker.
   * </p>
   */
  @Test
      ...
      for (String keyMarker : keyVersionMap.keySet()) {
        ...
        for (Map.Entry<String, List<String>> mapEntry : tailMap.entrySet()) {
          for (String versionId : mapEntry.getValue()) {
            S3VersionSummary versionSummary = summaryIterator.next();
            assertTrue("Expected versions to be lexicographically and chronologically ordered. Verification failed for key " + mapEntry.getKey()
                + ". Expected version ID " + versionId + ", but got " + versionSummary.getVersionId(), versionSummary.getVersionId()
                .equals(versionId));
            assertTrue("Expected delete marker to be set to false but found it to be true", !versionSummary.isDeleteMarker());
            verifyVersionCommonElements(versionSummary, mapEntry.getKey());
          }
  ...
  /**
   * <p>
   * Test for listing and verifying object versions using key-marker and version-id-marker.
   * </p>
   */
  @Test
        ...
        for (int i = 0; i < uploads; i++) {
          ...
          for (Map.Entry<String, List<String>> tailMapEntry : tailMap.entrySet()) {
            for (String versionId : tailMapEntry.getValue()) {
              S3VersionSummary versionSummary = summaryIterator.next();
              assertTrue(
                  "Expected versions to be lexicographically and chronologically ordered. Verification failed for key " + tailMapEntry.getKey()
  ...
  public void toggleVersioning() throws Exception {
    ...
    try {
      ...
      // Enable versioning
      print(account + ": Enabling versioning");
      s3.setBucketVersioningConfiguration(new SetBucketVersioningConfigurationRequest(bucketName, new BucketVersioningConfiguration()
      ...
      for (S3VersionSummary version : versions.getVersionSummaries()) {
        try {
          if (!version.isDeleteMarker()) {
            print(account + ": Deleting version element: key " + version.getKey() + ", version " + version.getVersionId());
            s3.deleteVersion(bucketName, version.getKey(), version.getVersionId());
          }
        } catch (AmazonServiceException ase) {
          error = true;
  ...
  public void cleanupmess() {
    ...
    print(account + ": Deleting bucket 3fc48edc955b2cf4");
  ...
  private void compare(LinkedList<KeyEntry> history, VersionListing versions) {
    ...
    while (versionIterator.hasNext()) {
      ...
      assertTrue("Mimatch in key type. Expected " + key.getElement() + " but got isDeleteMarker=" + version.isDeleteMarker(), key.getElement()
          .equals(Element.DELETE_MARKER) ? version.isDeleteMarker() : !version.isDeleteMarker());
    }
  }
  ...
  private void enableBucketVersioning(String bucketName) throws InterruptedException {
    ...
    do {
      ...
      print(account + ": Versioning state: " + versioning.getStatus());
    ...
    cleanupTasks.add(new Runnable() {
      ...
      public void run() {
        if (putResult.getVersionId() != null) {
          print(account + ": Deleting object " + key + ", version " + putResult.getVersionId());
  ...
  private void verifyVersionCommonElements(S3VersionSummary versionSummary, String key) {
    ...
    assertTrue("Expected owner ID to be " + ownerID + ", but got " + versionSummary.getOwner().getId(),
        versionSummary.getOwner().getId().equals(ownerID));
  }
