...
public class S3ObjectACLAcrossAccountsTests {
   ...
   * Bucket owner continues to have FULL_CONTROL permission on the object, object owner has no listed permissions. However the object owner seems to
  ...
  public void bucket_PublicReadWrite_object_BucketOwnerFullControl_1() throws Exception {
    ...
    try {
      ...
      while (iterator.hasNext()) {
        ...
        assertTrue(
            "Expected grantee to be object owner " + objectACL.getOwner().getId() + " or bucket owner " + s3ClientA.getS3AccountOwner().getId()
                + ", but found " + grant.getGrantee().getIdentifier(),
